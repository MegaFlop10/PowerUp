...
public class RightScaleRightScaleRight extends AutoMode {
	...
	public void runStep(int stepIndex) {
		...
		switch (stepIndex) {
			...
			case 2:
				...
				Drivetrain.getInstance().actionSensorDrive(0.4, 0.0, -0.5);
				break;
			...
			case 4:
				...
				Drivetrain.getInstance().actionSensorDrive(0.2, -155.0, 0.65);
				break;
			...
			case 7:
				Lift.getInstance().actionMoveTo(LiftPosition.SCALE_HI);
				Intake.getInstance().actionStow();
				Drivetrain.getInstance().actionSensorDrive(0.6, -15.0, 1.375);
				break;
				
			case 8:
				Lift.getInstance().actionMoveTo(LiftPosition.SCALE_HI);
				Intake.getInstance().actionOpenWhileStowed();
				Drivetrain.getInstance().actionSensorDrive(0.4, -15.0, 0.5);
				break;
	...
	public boolean getStepIsCompleted(int stepIndex) {
		...
		switch (stepIndex) {
			...
			case 6:
				if (Drivetrain.getInstance().getAngleWithinRange(-15.0, 3)) {
					Drivetrain.getInstance().setEncoderCounts(0);
					return true;
				}
				return false;
			...
			case 7:
				return Drivetrain.getInstance().getEncoderWithinDistance(1.375, 0.1);
